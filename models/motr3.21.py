    ...
    def match_for_single_frame(self, outputs: dict):
        ...
        if isinstance(gt_instances_i, list):
            if len(gt_instances_i) > 0:
                ...
                # 如果列表为空，创建一个空的Instances对象，但确保obj_ids不是空张量
                gt_instances_i = Instances((1, 1))
                device = outputs_without_aux['track_instances'].device
                # 创建一个长度为0但不是None的张量，避免后续索引错误
                gt_instances_i.obj_ids = torch.tensor([], device=device, dtype=torch.long)  # 确保是long类型
                gt_instances_i.boxes = torch.zeros((0, 4), device=device)
                gt_instances_i.labels = torch.zeros((0,), device=device, dtype=torch.long)
